...
| `roles/platform-engineer.md` | Platform / DevOps Engineer | High | S7 | Encodes invariants in tooling; gate results are deterministic and fail closed, while any release exception remains a separate human decision. |
...
| File | Role | Fit | Seat | Purpose |
| --- | --- | --- | --- | --- |
| `roles/release-manager.md` | Release Manager | Partial | S7 | Verifies gates from evidence not claims, defines the rollback threshold before deploying, and records every release exception with its human approver and expiry. |
...
| `roles/legal-contracts.md` | Legal / Contracts | Anchored | Borrowed | Assesses license-specific deployment, distribution, and network-use obligations with qualified counsel; reviews vendor data rights and contribution ownership. |
...
Two supporting rules: *deterministic gate results beat judgment* (a failure cannot be skipped,
relabeled, or argued into a pass; any policy-permitted release exception remains a separate,
explicitly human-approved decision), and *evidence beats assertion* ("tests pass" is not evidence
when command output can be supplied).
...
explicit write escalation, non-overridable deterministic gate results, and reviewer calibration from a
...
**Structurally validated, and enforced rather than asserted.** `scripts/validate-cards.sh` runs 26
...
```bash
bash scripts/validate-cards.sh     # All 26 structural checks passed.
```
